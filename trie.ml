(* module type Trie_sig = sig type t end

   module Trie = struct type t = { value: string; is_word: bool;
   words_containing: int; leaves: (string * t) list } end *)

(* the type of trie node: a pair of a boolean representing whether the
   node in the trie represents a word, and a list of pairs. Each element
   is a list is a pair of a string of a character and the child node
   that represents the new word formed with that character. *)
...
(* extracts the data type [string * t] from the option, returns empty
   pair if the option is none *)
let extract_st_option (o : (string * t) option) : string * t =
  match o with Some i -> i | None -> (" ", empty ())

(* extracts the data type [t] from the option, returns empty trie if the
   option is none *)
let extract_t_option (o : t option) : t =
  match o with Some i -> i | None -> empty ()

(* splitting a given string [input] into a list of letters of that word.
   No such String function does this, String.split_on_char needs a
   character with which to split*)
...
(* adds a new word to the trie. Takes in current trie and word. returns
   a new trie with the new word added. If the word node exists in the
   trie but is not marked as a word, then returns a trie with that word
   marked as a word.*)
let rec trie_insert (trie : t) (key : string list) : t =
  ...
  | stem :: rest ->
      ...
      in
      if
        matching_child = empty ()
        (* theoretically, every leaf node, i.e. node with no children in
           the trie should have is_word = true, so empty would not match
           an actual node in the trie *)
      then
        (* not in children, need to create new child node *)
        let new_child = trie_insert (Node (false, [])) rest in
        Node (is_word, children @ [ (stem, new_child) ])
      else
        (* yes in children,return current Node with children edited so
...
let rec trie_contains_word (trie : t) (key : string list) : bool =
  ...
  | stem :: rest ->
      ...
      if matching_child = empty () then
        (* not child has stem, doesnt contain*)
        false
      else
        (*child does contain stem, rec call again*)
        trie_contains_word matching_child rest

(* checks if input [key] is an actual node inside of the provided trie.
   [key] does not have to be a word! returns boolean *)
