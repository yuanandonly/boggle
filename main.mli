...
(** iterates through a list [l] of string lists and prints each string list *)
val printllist : string list list -> unit

(**  given a [word_list] string list, filters through and removes all words that 
are not in [possible_words] and therefore cannot be found from given board *)
...
(** takes in [input] int option and ensures that the player enters an input of 
either 1, 2, 3 to match game mode *)
val choose_game_mode : int option -> int

(** takes in string [name] and ensures that the single player's name does not 
conflict with integer names that represent computer player *)
...
(** takes in [input] int option and ensures that the player enters an input of 
either 1, 2, 3, 4, 5 to match computer player difficulty *)
...
(** provides option for player(s) to view all the words that the board contained
and displays their lengths and score values *)
...
(** iterates simultaneously through liast of player names [player_name_list] and
[score_list] and prints what each player scored *)
val multi_score_print : string list -> int list -> unit

(** takes in [input] int option and string list list [lst] of player-found words
...
calculatd by [time_length] from [time_start], calls the correct game_end 
function based on game mode as found from [player_name_list] *)
val countdown : board -> int -> float -> string list -> unit

(** queries player for board size and ensures valid input for the dimension, 
between 4 and 10 side length. takes in [input] int option and returns valid
size *)
