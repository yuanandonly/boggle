...
(* prompts user for list of words, and then prints out words nicely and scores
nicely, then says game over
For Later: add play again feature *)
...
let rec countdown (input_board : board) (time_length : int) 
  ...
  let _ = 
    ...
    match step with
    | 0 -> ANSITerminal.(print_string [cyan; Bold] ((clock_ascii0)^"\n"))
    | 1 -> ANSITerminal.(print_string [cyan; Bold] ((clock_ascii1)^"\n"))
    | 2 -> ANSITerminal.(print_string [cyan; Bold] ((clock_ascii2)^"\n"))
    | 3 -> ANSITerminal.(print_string [cyan; Bold] ((clock_ascii3)^"\n"))
    | 4 -> ANSITerminal.(print_string [cyan; Bold] ((clock_ascii4)^"\n"))
    | 5 -> ANSITerminal.(print_string [cyan; Bold] ((clock_ascii5)^"\n"))
    | 6 -> ANSITerminal.(print_string [cyan; Bold] ((clock_ascii6)^"\n"))
    | 7 -> ANSITerminal.(print_string [cyan; Bold] ((clock_ascii7)^"\n"))
    | _ -> ANSITerminal.(print_string [cyan; Bold] ((clock_ascii0)^"\n"))
  in
  if time_passed > (Float.of_int) time_length 
  then begin
...
let main () = 
  ...
  begin
    ...
    ANSITerminal.(print_string [green; Blink] 
      "How long would you like for the round? Provide a number in seconds. 
